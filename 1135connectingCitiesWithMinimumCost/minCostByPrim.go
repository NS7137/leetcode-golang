...
// Prim
func MinimumCostByPrim(n int, connections [][]int) int {
...
func buildGraph(n int, connections [][]int) [][]edge {
	graph := make([][]edge, n)
	for i := range graph {
		graph[i] = []edge{}
	}
	...
	for _, conn := range connections {
		...
		weight := conn[2]
		// 无向图就是双向图
		graph[from] = append(graph[from], edge{from, to, weight})
		graph[to] = append(graph[to], edge{to, from, weight})
	}
	return graph
}
